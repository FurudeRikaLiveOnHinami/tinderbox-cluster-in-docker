...
RUN echo -e "[binhost]\npriority = 9999\nsync-uri = $MIRROR_URL/experimental/amd64/binpkg/default/linux/17.1/x86-64/" | cat >> /etc/portage/binrepos.conf \
    && echo "EMERGE_DEFAULT_OPTS=\"--usepkg=y --getbinpkg=y --autounmask-write --autounmask-continue --autounmask-keep-keywords=y --autounmask-use=y\"" | cat >> /etc/portage/make.conf \
    ...
    && echo -e "dev-util/buildbot docker\ndev-db/postgresql -server" | cat >> /etc/portage/package.use/buildbot \
